...
import { IPatient, IPatientUpdate, BloodGroup, Gender } from '@/types/patients';

interface EditPatientModalProps {
  patient: IPatient | null;
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (id: string, data: IPatientUpdate) => Promise<void>;
}

export default function EditPatientModal({ patient, isOpen, onClose, onSubmit }: EditPatientModalProps) {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const [formData, setFormData] = useState<IPatientUpdate>({});
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await onSubmit(patient.id, payload);
      onClose();
    } catch (err: any) {
